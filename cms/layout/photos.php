...
$aa = sql_read('select * from product where status=? order by product_name ASC', 's', 1);
foreach($aa as $a){	
	$option['product'][$a['id']] = ucwords($a['product_name']);
}
...
$type['product'] = 'autosuggest'; $foreign_table['product'] = 'packages'; $foreign_field['product'] = 'package_name'; 
...
$style['product'] = array('width' => '100%'); //placeholder
...
if($_POST['action']=="Delete"){
	...
	if(!empty($_POST['productIdList'])){
		foreach($items_id_array as $items_id){
			$data['id']=$items_id;
...
<div class="col-12">
	...
	<?php if($keyword==true || $filter==true){?>
		<div class="row" style="margin:10px 0;">
		  ...
		  if($keyword==true){
		  $kyplaceholder="";$p=1;
			foreach($keywordFields as $f){ if($p!=1){$kyplaceholder.=", ";}$p++; $kyplaceholder.=$str_convert->to_eye($f);}?>
    ...
    <div id="tab" class="row">
        <ul class="nav nav-tabs">
            <?php foreach($option['status'] as $v => $l){
    ...
    <div class="nav-act">	
		<div class="row pt-2 pb-1">
			...
			<div class="col-auto">
			<?php }?>
			<?php foreach($actions as $action){?><input class="btn-check"  type="submit" name="action" value="<?php echo $action?>" onClick="return confirmAction('<?php echo $msg[$action]?>');" title="<?php echo $msg[$action]?> selected item(s)"><?php }?>
			</div>
		</div>
    </div>
    ...
    <div style="background:#DDD; border:none; border-bottom:2px solid #999;">
        ...
        <div class="col" style=" <?php if($edit==true){?>width:82%<?php }else{?>width:96%<?php }?> !important;">
            <?php foreach($cols as $colName => $colWidth){?>
	...
	foreach($rows as $key => $item){
	   $arr[$item['product']][$key] = $item;
	}
	ksort($arr, SORT_NUMERIC);
	
        	foreach($arr as $pack_id => $pval){?>
			...
			<?php 
			$product = sql_read('select * from product where id=?', 's', $pack_id);
	
			echo '<h4 style="color:#333;">'.$product['product_name'].'</h4>';
			?>
                 <div style="display:table-cell; width:50px;">
                    <input type="checkbox" value="" target-group="group<?php echo $product['id']?>" class="groupcheck" style=" border:1px solid red;">
                 ...
                 <div class="sortable" style="display:table-cell; border:1px dashed #CCC; background-color:#EFEFEF; padding:10px;">
                    ...
                    <?php foreach($pval as $val){?>
                      <div id="row<?php echo $val['id']?>" sort-group="sg-<?php echo $product['id']?>" class="sg-<?php echo $product['id']?>" 
                      style="display:inline-block; width:150px; margin:5px;" >	  
                          <div style="display:inline-block; width:100%; border:1px sold #CCC; padding:0">	
                              <div style="padding-left:14px;">
                                  <input type="checkbox" value="<?php echo $val['id']; ?>" name="productIdList[]" class="group<?php echo $product['id']?>">
