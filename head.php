...
require_once 'lpssdk/redeemable.php';
...
if(!empty($_SESSION['region'])){
    ...
    $region = sql_read("select * from region where status=1 and id='3' limit 1");//ORDER BY id asc
...
if(!function_exists('get_total_rebatible')){
    ...
    function get_total_rebatible(){
        ...
        if(@$_SESSION['auth_user']['id']){
            ...
            $rebates = sql_read("select rebate_earned, rebate_used from orders where member=? and payment_status='Paid' and status != 'Refund' ", 's', $_SESSION['auth_user']['id']);
...
if(!function_exists('shipping_fee')){
    function shipping_fee($item_total){
        
            $area_exist = sql_read('select * from area where id=? limit 1', 's', $_SESSION['area']);
            if(!empty($area_exist)) {
                $tier = sql_read('SELECT delivery_fee FROM delivery_tier WHERE status = ? AND area = ? AND ? BETWEEN min_checkout_price AND max_checkout_price LIMIT 1', 'iis', array(1, $_SESSION['area'], $item_total));
                
                if(!empty($tier)) {
                    $shipping_fee = $tier['delivery_fee'];
                } else {
                    $shipping_fee = $area_exist['price'];
                }
            } else {
                $shipping_fee = 0;
            }
...
*/?>
    ...
    <script src="lpssdk/lps.js"></script>
